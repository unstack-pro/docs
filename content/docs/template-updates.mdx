...
Template syncing is not available for this edition yet.
...
Adds the template repository as a `template` remote (one-time) and fetches it.
</Step>
...
- **`lib/auth-client.ts`** — template changes here are usually improvements;
  accept unless you've customized it.
- **`app/*`, `components/*`** — if you've customized heavily, review carefully.

The data and auth layer is where the two editions diverge:

<Tabs groupId="edition" persist items={["Postgres", "Convex"]}>

<Tab value="Postgres">

`lib/auth.ts`, `lib/db/*` and `actions/*` — accept unless you've customized them.
A new file under `drizzle/` is a migration and is purely additive: take it, then
...
`convex/*` — accept unless you've customized it. Schema changes are pushed by
`bun convex dev`, so there is no migration file to apply afterwards.
