...
5. **Generate JWKS keys for production**:
   ```bash
   bunx convex run betterAuth/auth:rotateKeys | bunx convex env set JWKS --prod
   ```
...
- [ ] `JWKS` (generated with `bunx convex run betterAuth/auth:rotateKeys | bunx convex env set JWKS --prod`)
