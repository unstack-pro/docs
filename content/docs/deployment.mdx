...
3. **Set production environment variables**:
   ...
   npx convex env set RESEND_API_KEY "your-resend-key" --prod
   npx convex env set EMAIL_FROM "Your App <no-reply@yourdomain.com>" --prod
   npx convex env set AUTUMN_API_KEY "your-autumn-key" --prod
   ```
...
4. **Add environment variables**:
   ...
   # Passkeys
   PASSKEY_RP_ID=yourdomain.vercel.app
   
   # Organizations (required)
   AUTUMN_API_KEY=your-autumn-prod-key
   ```
...
3. Configure in these files:
   - `instrumentation.ts` (client-side)
...
- [ ] `AUTUMN_API_KEY` (required for organizations)
...
For better performance, use Edge Runtime where possible:
...
Vercel automatically enables compression. No configuration needed.
